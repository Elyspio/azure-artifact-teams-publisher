...
export class TokenClient {
    ...
    getToken(organisation: string , cancelToken?: CancelToken | undefined): Promise<Config> {
        let url_ = this.baseUrl + "/api/config/{organisation}";
        if (organisation === undefined || organisation === null)
            throw new Error("The parameter 'organisation' must be defined.");
        url_ = url_.replace("{organisation}", encodeURIComponent("" + organisation));
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processGetToken(response: AxiosResponse): Promise<Config> {
        const status = response.status;
        let _headers: any = {};
        if (response.headers && typeof response.headers === "object") {
            for (let k in response.headers) {
    ...
    setToken(organisation: string, config: ConfigBase , cancelToken?: CancelToken | undefined): Promise<void> {
        let url_ = this.baseUrl + "/api/config/{organisation}";
        if (organisation === undefined || organisation === null)
            throw new Error("The parameter 'organisation' must be defined.");
        url_ = url_.replace("{organisation}", encodeURIComponent("" + organisation));
...
export interface ArtifactRepositoryId {
    project: string;
    repository: string;
}
